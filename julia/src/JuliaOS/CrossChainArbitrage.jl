...
function execute_cross_chain_trade(
    ...
    @info "Executing cross-chain trade: $source_chain -> $target_chain for $amount $token"
    
    try
        # Step 1: Get accurate gas estimates from source chain
        gas_estimate = Bridge.send_command("blockchain", "getGasPrice", Dict(
            "chain" => source_chain
        ))
        
        if !haskey(gas_estimate, "success") || !gas_estimate["success"]
            @warn "Failed to get gas price for $source_chain"
            return Dict(
                "success" => false,
        ...
        gas_price = gas_estimate["data"]["price"]
        @info "Gas price for $source_chain: $gas_price"
        
        # Step 2: Execute the transaction on source chain
        swap_tx = Bridge.send_command("dex", "swap", Dict(
            "chain" => source_chain,
            "token" => token,
            "amount" => amount,
            "gasPrice" => gas_price,
            "walletAddress" => source_info.wallet_address
        ))
        
        if !haskey(swap_tx, "success") || !swap_tx["success"]
            @warn "Failed to execute swap on $source_chain"
            return Dict(
                "success" => false,
                "error" => "Failed to execute source chain swap",
                "timestamp" => now()
            )
        end
        
        source_tx_hash = swap_tx["data"]["txHash"]
        @info "Source chain swap executed: $source_tx_hash"
        
        # Step 3: Execute bridge transaction
        bridge_tx = Bridge.send_command("bridge", "transfer", Dict(
            "sourceChain" => source_chain,
            "targetChain" => target_chain,
            "token" => token,
            "amount" => amount,
            "sourceWallet" => source_info.wallet_address,
            "targetWallet" => target_info.wallet_address
        ))
        
        if !haskey(bridge_tx, "success") || !bridge_tx["success"]
            @warn "Failed to execute bridge transfer"
            return Dict(
                "success" => false,
                "error" => "Failed to execute bridge transfer",
                "timestamp" => now()
            )
        end
        
        bridge_tx_hash = bridge_tx["data"]["txHash"]
        bridge_fee = bridge_tx["data"]["fee"]
        @info "Bridge transfer executed: $bridge_tx_hash with fee $bridge_fee"
        
        # Step 4: Wait for and verify the completion on target chain
        completion_status = Bridge.send_command("bridge", "getStatus", Dict(
            "txHash" => bridge_tx_hash,
            "targetChain" => target_chain
        ))
        
        timeout_counter = 0
        max_timeout = 60 # 5 minutes in 5-second increments
        
        # Poll for completion status with timeout
        while (!haskey(completion_status, "success") || 
               !completion_status["success"] || 
               completion_status["data"]["status"] != "completed") && timeout_counter < max_timeout
            ...
            if timeout_counter % 12 == 0 # Log every minute
                @info "Waiting for bridge completion... ($(timeout_counter ÷ 12) minutes elapsed)"
            end
        end
        
        if timeout_counter >= max_timeout
            @warn "Timeout waiting for bridge completion"
            return Dict(
                "success" => false,
                "error" => "Timeout waiting for bridge completion",
                "timestamp" => now(),
        ...
        target_tx_hash = completion_status["data"]["targetTransactionHash"]
        @info "Bridge transfer completed on target chain: $target_tx_hash"
        
        # Step 5: Calculate the final profit based on exchange rates
        source_price = Bridge.send_command("market", "getPrice", Dict(
            "chain" => source_chain,
            "token" => token
        ))["data"]["price"]
        
        target_price = Bridge.send_command("market", "getPrice", Dict(
            "chain" => target_chain,
            "token" => token
        ))["data"]["price"]
        
        # Calculate profit (including gas and bridge fees)
        source_value = amount * source_price
        target_value = amount * target_price
        total_fees = bridge_fee + gas_estimate["data"]["gasLimit"] * gas_price
        
        profit = target_value - source_value - total_fees
        ...
        return Dict(
            ...
            "profit" => profit,
            "gas_used" => gas_estimate["data"]["gasLimit"] * gas_price,
            "bridge_fee" => bridge_fee,
            "timestamp" => now()
        )
    catch e
        @error "Error executing cross-chain trade" exception=(e, catch_backtrace())
        return Dict(
            "success" => false,
            "error" => "Exception: $e",
            "timestamp" => now()
