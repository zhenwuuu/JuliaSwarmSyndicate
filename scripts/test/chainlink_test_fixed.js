...
// Helper function to format price from BigInt
function formatPrice(answer, decimals) {
  const answerStr = answer.toString();
  const divisor = BigInt(10) ** BigInt(decimals);
  const wholePart = BigInt(answerStr) / divisor;
  const fractionalPart = BigInt(answerStr) % divisor;
  ...
  try {
    ...
    for (const [pair, address] of Object.entries(CHAINLINK_FEEDS)) {
      ...
      try {
        ...
        const priceStr = formatPrice(roundData.answer, decimals);

        // Calculate data age
        const updatedAtBigInt = roundData.updatedAt;
        const updatedAt = Number(updatedAtBigInt) * 1000; // Convert to milliseconds
        const now = Date.now();
